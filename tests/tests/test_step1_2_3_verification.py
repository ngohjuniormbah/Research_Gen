...
async def test_step1_export_download_flow(client: AsyncClient, auth_headers: dict) -> None:
    ...
    # 3. Simulate frontend target resolution (Step 1 fix)
    api_base = "http://test"  # local test base
    target_url = download_url if download_url.startswith(
        "http") else f"{api_base}{download_url}"

    # 4. Fetch the signed download URL (must return 200 with PDF content)
...
class Mock48ComparisonClient(ORKGClient):
    ...
    async def get_statements(
        ...
        if subject_id == "R_MALARIA_48":
            # Return 48 comparison contributions
            return [
                {
        ...
        if subject_id.startswith("C_"):
            ...
            return [
                {"predicate": {"label": "Method"}, "object": {
                    "label": f"Drug-Regimen-{i}", "_class": "literal"}},
                {"predicate": {"label": "Efficacy"}, "object": {
                    "label": f"9{int(i) % 10}.5%", "_class": "literal"}},
                {"predicate": {"label": "Dataset"}, "object": {
                    "label": f"Cohort-{i}", "_class": "literal"}},
                {"predicate": {"label": "year"}, "object": {
                    "label": str(2020 + (int(i) % 5)), "_class": "literal"}},
                {"predicate": {"label": "doi"}, "object": {
                    "label": f"10.1000/trial_{i}", "_class": "literal"}},
            ]
        return []
...
async def test_step2_resolve_48_comparison_tables() -> None:
    ...
    # 1 master comparison record + 48 individual unpacked studies = 49 records
    assert len(records) == 49

    master = records[0]
    assert master["resolved"] is True
    # Verify that the 48th contribution was NOT dropped (old limit was 15)
    ...
    # Verify that individual unpacked records exist for each study
...
def test_step3_context_budget_handles_48_tables() -> None:
    ...
    # Confirm config expansion
    assert settings.llm_max_context_tokens >= 32000

    # Create 48 records, each holding an empirical comparison table
    records = [
        SourceRecord(
                ...
                "tables": [
                    {
                        ...
                        "rows": [
                            ...
                            [f"Model-Variant-{i}", f"Benchmark-{i}",
                                "Accuracy", f"9{i % 10}.8%"],
                            [f"Baseline-{i}", f"Benchmark-{i}",
                                "Accuracy", f"8{i % 10}.1%"],
                        ],
                    }
                ]
    ...
    bundle = build_context(
        records, token_budget=settings.llm_max_context_tokens)

    # Strategy must be 'direct' (no forced map-reduce dropping)
    assert bundle.strategy == "direct"
    assert bundle.included == 48
    assert bundle.dropped == 0

    # Ensure every single table row was rendered into the prompt
    for i in range(1, 49):
